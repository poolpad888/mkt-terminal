...
// Из раздела вытаскиваем ссылки на выпуски и берём самую позднюю по дате.
function ссылкиРосстата(html) {
  const rx = /(?:href=["'])([^"']*mediabank\/\d+_(\d{2})-(\d{2})-(\d{4})\.html)/gi;
  ...
  while ((m = rx.exec(html))) {
    ...
    было.add(url);
    out.push({ url, дата: `${m[4]}-${m[3]}-${m[2]}` });
  }
  return out.sort((a, b) => a.дата < b.дата ? 1 : -1);
}
...
async function обновитьИПЦ() {
  ...
  const html = await fetchUrl(ссылки[0].url);
  const д = разборИПЦ(html);
  if (!д) throw new Error('не нашёл строку с оценкой в выпуске ' + ссылки[0].url);
  ИПЦ = { данные: д, время: Date.now(), адрес: ссылки[0].url };
  return ИПЦ;
}
