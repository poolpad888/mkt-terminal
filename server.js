...
// ── Остальные показатели Росстата ───────────────────────────────────────
// Инфляцию разбирали отдельно и давно; здесь — всё прочее. Вид определяем по
// первой строке выпуска: она же заголовок. Из текста берём одно число, то,
// которое имеет смысл поставить в календарь рядом с прошлым выпуском.
// Числа в выпусках Росстата иногда разорваны пробелами («101,3 %», «56 166,3»),
// поэтому пробелы внутри чисел терпим.
const Ч = '([\\d]+(?:[\\s\\u00a0][\\d]{3})*(?:,[\\d]+)?)';
const чист = s => вЧисло(String(s).replace(/[\s\u00a0]/g, ''));

const РОССТАТ_РАЗБОР = [
  { вид: 'gdp', имя: /о\s+производстве\s+валового\s+внутреннего\s+продукта/i,
    брать: т => {
      const m = new RegExp('Индекс\\s+его\\s+физического\\s+объема\\s+относительно[^%]{0,80}?составил\\s+' + Ч + '\\s*%', 'i').exec(т);
      return m ? знак(вПроцент(чист(m[1]))) : '';
    } },
  { вид: 'gdpp', имя: /предварительн\w*\s+оценк\w*\s+ввп|о\s+производстве\s+валового/i,
    брать: т => {
      const m = new RegExp('Индекс\\s+его\\s+физического\\s+объема\\s+относительно[^%]{0,80}?составил\\s+' + Ч + '\\s*%', 'i').exec(т);
      return m ? знак(вПроцент(чист(m[1]))) : '';
    } },
  { вид: 'ind', имя: /о\s+промышленном\s+производстве|индекс\w*\s+промышленного\s+производства/i,
    брать: т => {
      const m = new RegExp('индекс\\s+промышленного\\s+производства[^%]{0,140}?составил\\s+' + Ч + '\\s*%', 'i').exec(т);
      return m ? знак(вПроцент(чист(m[1]))) : '';
    } },
  { вид: 'ppi', имя: /о\s+ценах\s+производителей|индекс\w*\s+цен\s+производителей/i,
    брать: т => {
      const m = new RegExp('индекс\\s+цен\\s+производителей[^%]{0,140}?составил\\s+' + Ч + '\\s*%', 'i').exec(т);
      return m ? знак(вПроцент(чист(m[1]))) : '';
    } },
  { вид: 'zpl', имя: /о\s+просроченной\s+задолженности\s+по\s+заработной\s+плате/i,
    брать: т => {
      const m = new RegExp('задолженность\\s+по\\s+заработной\\s+плате\\s+составила\\s+' + Ч + '\\s*млн', 'i').exec(т);
      return m ? Math.round(чист(m[1])).toLocaleString('ru-RU').replace(/\u00a0/g, ' ') : '';
    } },
  { вид: 'prof', имя: /о\s+финансовых\s+результатах\s+деятельности\s+организаций/i,
    брать: т => {
      const m = new RegExp('в\\s+действующих\\s+ценах\\s+составил\\s+' + Ч + '\\s*млрд', 'i').exec(т);
      return m ? Math.round(чист(m[1])).toLocaleString('ru-RU').replace(/\u00a0/g, ' ') : '';
    } },
];

const РОССТАТ_ФАЙЛ = path.join(__dirname, 'rosstat.json');
let РОССТАТ_ФАКТЫ = { факты: {}, разобрано: {} };
try { Object.assign(РОССТАТ_ФАКТЫ, JSON.parse(fs.readFileSync(РОССТАТ_ФАЙЛ, 'utf8')) || {}); } catch (e) {}
function росстатСохранить() {
  try {
    fs.writeFileSync(РОССТАТ_ФАЙЛ + '.tmp', JSON.stringify(РОССТАТ_ФАКТЫ));
    fs.renameSync(РОССТАТ_ФАЙЛ + '.tmp', РОССТАТ_ФАЙЛ);
  } catch (e) {}
}

// Из текста выпуска достаём вид и значение. Если правило не сошлось —
// возвращаем пустое: пустое поле честнее выдуманного числа.
function разборРосстата(html) {
  const т = текстСтраницы(html).replace(/\s+/g, ' ');
  // «I I квартал» — следствие разрыва римской цифры при вычистке разметки.
  const шапка = т.slice(0, 300);
  for (const п of РОССТАТ_РАЗБОР) {
    if (!п.имя.test(шапка)) continue;
    const v = п.брать(т);
    if (v) return { вид: п.вид, значение: v };
  }
  return null;
}

// Обход раздела для всего, кроме инфляции: её разбирает ипцОбойти.
async function росстатОбойти(предел = 10) {
  const раздел = await fetchUrl(РОССТАТ_РАЗДЕЛ);
  const ссылки = ссылкиРосстата(раздел);
  let новых = 0, ошибок = 0, мимо = 0;
  for (const с of ссылки) {
    if (РОССТАТ_ФАКТЫ.разобрано[с.url]) continue;
    if (новых + ошибок >= предел) break;
    let html;
    try { html = await fetchUrl(с.url); } catch (e) { ошибок++; continue; }
    const р = разборРосстата(html);
    if (р) {
      РОССТАТ_ФАКТЫ.факты[р.вид] = РОССТАТ_ФАКТЫ.факты[р.вид] || {};
      РОССТАТ_ФАКТЫ.факты[р.вид][с.дата] = р.значение;
      новых++;
    } else мимо++;
    РОССТАТ_ФАКТЫ.разобрано[с.url] = с.дата;
  }
  росстатСохранить();
  return { новых, ошибок, мимо, видов: Object.keys(РОССТАТ_ФАКТЫ.факты).length };
}
setTimeout(() => { росстатОбойти(10).catch(() => {}); }, 70 * 1000);
setInterval(() => { росстатОбойти(6).catch(() => {}); }, 60 * 60 * 1000);
...
// ── Аукционы ОФЗ: сбор с сайта Минфина ──────────────────────────────────
// Во вторник Минфин сообщает, какие выпуски разместит в среду, — это и есть
// главная новость дня по госдолгу. Сообщение появляется во второй половине
// дня, обычно около 15:45, но может задержаться. Поэтому в это окно
// спрашиваем сайт часто, а в остальное время — изредка, чтобы подобрать
// пропущенное после перезапуска.
const ОФЗ = require('./ofz.js');
const ОФЗ_ФАЙЛ = path.join(__dirname, 'ofz.json');
let ОФЗ_ДАННЫЕ = { объявления: {}, итоги: {} };
try { Object.assign(ОФЗ_ДАННЫЕ, JSON.parse(fs.readFileSync(ОФЗ_ФАЙЛ, 'utf8')) || {}); } catch (e) {}
function офзСохранить() {
  try {
    fs.writeFileSync(ОФЗ_ФАЙЛ + '.tmp', JSON.stringify(ОФЗ_ДАННЫЕ));
    fs.renameSync(ОФЗ_ФАЙЛ + '.tmp', ОФЗ_ФАЙЛ);
  } catch (e) {}
}

// Московские сутки и время. Сервер может стоять в любом часовом поясе,
// поэтому день недели и часы считаем только через часовой пояс Москвы.
function мскЧасти(t = Date.now()) {
  const d = new Date(t);
  const ymd = d.toLocaleDateString('sv-SE', { timeZone: 'Europe/Moscow' });
  const чм = d.toLocaleTimeString('ru-RU', { timeZone: 'Europe/Moscow', hour12: false,
                                             hour: '2-digit', minute: '2-digit' });
  const [ч, м] = чм.split(':').map(Number);
  const дн = new Date(ymd + 'T12:00:00+03:00').getUTCDay();
  return { ymd, ч, м, дн, минут: ч * 60 + м };
}
const сдвигДня = (ymd, дней) =>
  new Date(new Date(ymd + 'T12:00:00+03:00').getTime() + дней * 864e5)
    .toLocaleDateString('sv-SE', { timeZone: 'Europe/Moscow' });

// Один заход: ищем в разделе сообщение про аукционы назначенной даты и,
// если нашли, читаем его и запоминаем выпуски. Ключ — день объявления
// (вторник), потому что именно на этот день заведено событие в календаре.
async function офзОбъявление(деньОбъявления) {
  const датаАукциона = сдвигДня(деньОбъявления, 1);
  if (ОФЗ_ДАННЫЕ.объявления[деньОбъявления]) return { уже: true };
  const списк = await fetchUrl(ОФЗ.РАЗДЕЛ);
  const с = ОФЗ.найтиОбъявление(списк, датаАукциона);
  if (!с) return { нет: true };
  if (с.отменён) {
    ОФЗ_ДАННЫЕ.объявления[деньОбъявления] = { bonds: 'аукционы не проводятся', url: с.url };
    офзСохранить();
    return { отменён: true };
  }
  const док = await fetchUrl(с.url);
  const р = ОФЗ.разборОбъявления(док);
  // Если разметка изменилась и выпуски не нашлись — ничего не пишем: пустое
  // поле честнее выдуманного.
  if (!р.строка) return { разметка: true };
  ОФЗ_ДАННЫЕ.объявления[деньОбъявления] = { bonds: р.строка, url: с.url };
  офзСохранить();
  return { записано: р.строка };
}

// Среда — итоги. Минфин кладёт отдельный документ на каждый выпуск, поэтому
// собираем все документы этой даты и складываем размещённые объёмы. Пока
// вышел только первый аукцион, сумма будет неполной — поэтому запись
// считаем готовой лишь тогда, когда документов столько же, сколько выпусков
// объявляли накануне.
async function офзИтоги(датаАукциона) {
  const готово = ОФЗ_ДАННЫЕ.итоги[датаАукциона];
  if (готово && готово.полно) return { уже: true };
  const объявл = ОФЗ_ДАННЫЕ.объявления[сдвигДня(датаАукциона, -1)] || {};
  const ждём = (объявл.bonds || '').split(/,| и /).filter(x => /\d{5}/.test(x)).length;
  const списк = await fetchUrl(ОФЗ.РАЗДЕЛ);
  const ссылки = ОФЗ.найтиРезультаты(списк, датаАукциона);
  if (!ссылки.length) return { нет: true };
  let номинал = 0, выручка = 0;
  const выпуски = [];
  for (const с of ссылки) {
    const док = await fetchUrl(с.url);
    const р = ОФЗ.разборРезультатов(док);
    if (р.номинал == null) continue;   // разметку поменяли — число не выдумываем
    номинал += р.номинал;
    выручка += р.выручка || 0;
    выпуски.push(р.выпуск);
  }
  if (!выпуски.length) return { разметка: true };
  // Документ о дополнительном размещении выходит позже двух основных, поэтому
  // в течение самого дня аукциона запись держим открытой и перечитываем.
  // Закрываем её вечером или когда день уже прошёл.
  const t = мскЧасти();
  const деньЗакрыт = датаАукциона < t.ymd || t.минут >= 20 * 60;
  ОФЗ_ДАННЫЕ.итоги[датаАукциона] = {
    fact: ОФЗ.вЧисло(номинал), выручка: ОФЗ.вЧисло(выручка),
    выпуски, документов: выпуски.length,
    полно: деньЗакрыт && (ждём ? выпуски.length >= ждём : выпуски.length > 1),
  };
  офзСохранить();
  return ОФЗ_ДАННЫЕ.итоги[датаАукциона];
}

// Сторож: во вторник с 15:40 до 18:30 спрашиваем раз в 15 секунд, пока не
// найдём. В остальное время — раз в полчаса, на случай пропуска.
let офзПоследний = 0, итогиПоследний = 0;
async function офзТик() {
  const t = мскЧасти();
  const окноВт = t.дн === 2 && t.минут >= 15 * 60 + 40 && t.минут <= 18 * 60 + 30;
  // Первый аукцион подводят к 14:00, второй к 16:00; документы появляются
  // следом, иногда с задержкой до вечера.
  const окноСр = t.дн === 3 && t.минут >= 13 * 60 + 50 && t.минут <= 20 * 60;
  const пауза = (окноВт || окноСр) ? 15 * 1000 : 30 * 60 * 1000;
  if (Date.now() - офзПоследний < пауза) return;
  офзПоследний = Date.now();
  // Во вторник смотрим сегодняшнее объявление, в остальные дни — ближайшее
  // прошедшее, чтобы заполнить пропуск после перезапуска.
  const вторник = t.дн === 2 ? t.ymd : сдвигДня(t.ymd, -((t.дн + 5) % 7));
  try { await офзОбъявление(вторник); } catch (e) {}
  // Итоги: в среду свои, в другие дни добираем ближайшую прошедшую среду.
  // Три документа за заход — спрашиваем реже, чем объявление: раз в две
  // минуты в день аукциона и раз в полчаса в остальное время.
  const среда = сдвигДня(вторник, 1);
  const паузаИт = окноСр ? 2 * 60 * 1000 : 30 * 60 * 1000;
  if (среда <= t.ymd && Date.now() - итогиПоследний >= паузаИт) {
    итогиПоследний = Date.now();
    try { await офзИтоги(среда); } catch (e) {}
  }
}
setInterval(() => { офзТик().catch(() => {}); }, 5 * 1000);
setTimeout(() => { офзТик().catch(() => {}); }, 50 * 1000);
...
// Ставка, действующая на дату: последнее известное значение не позже её.
...
// Итог заседания: значение на первый же день после него. Брать «сегодняшнюю»
// ставку нельзя — для старых заседаний это дало бы нынешний уровень, а не тот,
// который тогда установили.
function ставкаПосле(ряд, ymd) {
  const дни = Object.keys(ряд).sort();
  for (const д of дни) if (д > ymd) return ряд[д];
  return '';
}

// Запасной путь: решение объявляют в 13:30, а в таблице новая ставка
// появляется только с первого рабочего дня её действия — в пятничных
// заседаниях это понедельник. Чтобы факт стоял в тот же день, читаем
// заявление председателя: «...снизить ключевую ставку до 13,50% годовых».
async function ставкаИзЗаявления(датаЗаседания) {
  const раздел = await fetchUrl(ЦБ_РАЗДЕЛ);
  const стр = цбЗаявления(раздел)[датаЗаседания];
  if (!стр) return '';
...
async function ставкиОбойти() {
  ...
  let вписано = 0;
  const сегодня = мскЧасти().ymd;
  for (const стр of CAL_CBR) {
    const дата = стр[0];
    const до = сдвигДня(дата, -1);
    if (дата > сегодня) continue;                     // заседание ещё не прошло
    const пред = ставкаНа(ряд, до);
    let факт = ставкаПосле(ряд, дата);
    // Прошло, а в таблице ещё нет — смотрим заявление председателя.
    if (!факт) { try { факт = await ставкаИзЗаявления(дата); } catch (e) {} }
...
function calRules(начало, конец) {
  ...
  for (let d = new Date(d0); d <= d1; d.setDate(d.getDate() + 1)) {
    ...
    if (дн === 2) {
      const о = ОФЗ_ДАННЫЕ.объявления[ymd] || {};
      ...
      out.push({ id: 'ofzann-' + ymd, date: ymd, time: '15:45', kind: 'fin',
                 ...
                 prev: (ОФЗ_ДАННЫЕ.объявления[прошлыйВторник] || {}).bonds || '',
    ...
    if (дн === 3) {
      const ф = ОФЗ_ДАННЫЕ.итоги[ymd] || CAL_OFZ_FACT[ymd] || {};
      ...
      const прошлаяСреда = new Date(d.getTime() - 7 * 864e5)
        .toLocaleDateString('sv-SE', { timeZone: 'Europe/Moscow' });
      const пред = ф.prev || (ОФЗ_ДАННЫЕ.итоги[прошлаяСреда] || {}).fact
                          || (CAL_OFZ_FACT[прошлаяСреда] || {}).fact || '';
      ...
      out.push({ id: 'ofz-' + ymd, date: ymd, time: '16:00', kind: 'fin', who: 'Минфин России', cc: 'ru',
                 name: 'Аукционы ОФЗ', note: 'размещено за день, оба аукциона',
...
function calEvents(начало, конец) {
  ...
  for (const [d, вид, период] of [...CAL_ROSSTAT_ПРОШЛОЕ, ...CAL_ROSSTAT]) {
    ...
    if (!в) continue;
    // Инфляция — своя, давняя таблица; остальное берём из общего сборщика.
    const табл = (вид === 'ipcw' || вид === 'ipcm') ? ипцТаблица(вид)
               : (РОССТАТ_ФАКТЫ.факты[вид] || null);
  ...
  try {
    ...
    if (u.pathname === '/api/rosstat') {
      try {
        ...
        if (u.searchParams.get('reset')) { РОССТАТ_ФАКТЫ = { факты: {}, разобрано: {} }; росстатСохранить(); }
    ...
    if (u.pathname === '/api/ofzres') {
      ...
      try {
        ...
        for (const с of ссылки) {
          ...
          const i = т.search(/Итоги\s+размещения|Минфин\s+России\s+информирует/i);
          разбор.push({ url: с.url, ...ОФЗ.разборРезультатов(док),
                        кусок: i >= 0 ? т.slice(i, i + 400) : т.slice(0, 300) });
        }
        const сумма = разбор.reduce((a, р) => a + (р.номинал || 0), 0);
        return sendJson(req, res, { ok: true, дата, найдено: ссылки.length,
                                    сумма: ОФЗ.вЧисло(сумма), разбор },
                        { 'Cache-Control': 'no-store' });
      } catch (e) {
        return sendJson(req, res, { ok: false, ошибка: e.message }, { 'Cache-Control': 'no-store' });
      }
    ...
    if (u.pathname === '/api/ofz') {
      try {
        const день = (u.searchParams.get('read') || '').slice(0, 10);
        let итог = null;
        if (день) {
          delete ОФЗ_ДАННЫЕ.объявления[день];
          delete ОФЗ_ДАННЫЕ.итоги[сдвигДня(день, 1)];
          итог = await офзОбъявление(день);
          const ит = await офзИтоги(сдвигДня(день, 1));
          итог = { объявление: итог, итоги: ит };
        }
        return sendJson(req, res, { ok: true, итог, данные: ОФЗ_ДАННЫЕ },
                        { 'Cache-Control': 'no-store' });
      } catch (e) {
        return sendJson(req, res, { ok: false, ошибка: e.message }, { 'Cache-Control': 'no-store' });
      }
