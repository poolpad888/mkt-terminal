...
// ── Росстат: недельная инфляция ─────────────────────────────────────────
// Выпуск выходит по средам в 19:00 МСК. Адрес выпуска содержит сквозной
// номер, заранее его не угадать, поэтому сначала берём раздел со списком
// и вытаскиваем самую свежую ссылку вида mediabank/НОМЕР_ДД-ММ-ГГГГ.html.
const РОССТАТ_РАЗДЕЛ = 'https://rosstat.gov.ru/compendium/document/50798';
const РОССТАТ_МЕС = { 'января':1,'февраля':2,'марта':3,'апреля':4,'мая':5,'июня':6,
  'июля':7,'августа':8,'сентября':9,'октября':10,'ноября':11,'декабря':12 };

function текстСтраницы(h) {
  let t = h.replace(/<script[\s\S]*?<\/script>/gi, '').replace(/<style[\s\S]*?<\/style>/gi, '');
  t = t.replace(/<[^>]+>/g, ' ');
...
// Росстат печатает индекс (99,99%), а в новостях принято изменение (-0,01%).
const вПроцент = i => Math.round((i - 100) * 100) / 100;
const вЧисло = s => parseFloat(String(s).replace(',', '.'));

function разборИПЦ(html) {
  const t = текстСтраницы(html);
  const о = t.match(/индекс потребительских цен[^,]*,\s*по оценке Росстата,\s*составил\s+([\d,]+)\s*%[^%]*?с начала месяца\s*[–—-]\s*([\d,]+)\s*%[^%]*?с начала года\s*[–—-]\s*([\d,]+)\s*%/i);
  if (!о) return null;
  // Период пишут двояко: «с 30 июня по 6 июля 2026» и «с 25 по 31 августа 2026»
  let период = null;
  const п1 = t.match(/за период с\s+(\d{1,2})\s+([а-яё]+)\s+по\s+(\d{1,2})\s+([а-яё]+)\s+(\d{4})/i);
  const п2 = t.match(/за период с\s+(\d{1,2})\s+по\s+(\d{1,2})\s+([а-яё]+)\s+(\d{4})/i);
  if (п1) период = { с: +п1[1], месС: РОССТАТ_МЕС[п1[2].toLowerCase()], по: +п1[3], мес: РОССТАТ_МЕС[п1[4].toLowerCase()], год: +п1[5] };
  else if (п2) период = { с: +п2[1], месС: РОССТАТ_МЕС[п2[3].toLowerCase()], по: +п2[2], мес: РОССТАТ_МЕС[п2[3].toLowerCase()], год: +п2[4] };
  const спр = t.match(/справочно:\s*([а-яё]+)\s+(\d{4})[^%]*?[–—-]\s*([\d,]+)\s*%,\s*с начала года\s*[–—-]\s*([\d,]+)\s*%/i);
  return {
    период,
    неделя: вПроцент(вЧисло(о[1])),
    месяц: вПроцент(вЧисло(о[2])),
    год: вПроцент(вЧисло(о[3])),
    индексы: { неделя: вЧисло(о[1]), месяц: вЧисло(о[2]), год: вЧисло(о[3]) },
    справочно: спр ? { мес: спр[1], год: +спр[2], месяц: вПроцент(вЧисло(спр[3])), год_: вПроцент(вЧисло(спр[4])) } : null,
  };
}

// Из раздела вытаскиваем ссылки на выпуски вместе с их названиями. В один
// день Росстат кладёт несколько документов (инфляция, нефтепродукты и т.д.),
// поэтому одной даты мало — сначала идут ссылки с нужным заголовком.
function ссылкиРосстата(html) {
  const rx = /<a\b[^>]*href=["']([^"']*mediabank\/\d+_(\d{2})-(\d{2})-(\d{4})\.html)["'][^>]*>([\s\S]{0,400}?)<\/a>/gi;
  const было = new Set(); const out = [];
  let m;
  while ((m = rx.exec(html))) {
    const url = new URL(m[1], 'https://rosstat.gov.ru/').href;
    if (было.has(url)) continue;
    было.add(url);
    const имя = текстСтраницы(m[5]);
    out.push({ url, дата: `${m[4]}-${m[3]}-${m[2]}`, имя, ипц: /индекс(а|е)? потребительских цен/i.test(имя) });
  }
  // без заголовков (голые ссылки) — запасной разбор
  if (!out.length) {
    const rx2 = /href=["']([^"']*mediabank\/\d+_(\d{2})-(\d{2})-(\d{4})\.html)/gi;
    while ((m = rx2.exec(html))) {
      const url = new URL(m[1], 'https://rosstat.gov.ru/').href;
      if (было.has(url)) continue;
      было.add(url);
      out.push({ url, дата: `${m[4]}-${m[3]}-${m[2]}`, имя: '', ипц: false });
    }
  }
  return out.sort((a, b) => (b.ипц - a.ипц) || (a.дата < b.дата ? 1 : -1));
}

// Месячный выпуск написан иначе: «Индекс потребительских цен в июле 2026 г.
// к июню 2026 г. составил 100,54%, к декабрю 2025 г. – 104,75%».
function разборИПЦМесяц(html) {
  const t = текстСтраницы(html);
  const о = t.match(/индекс потребительских цен в\s+([а-яё]+)\s+(\d{4})[^%]*?составил\s+([\d,]+)\s*%[^%]*?к декабрю\s+\d{4}[^%]*?[–—-]\s*([\d,]+)\s*%/i);
  if (!о) return null;
  return { месяц: о[1], год: +о[2], кМесяцу: вПроцент(вЧисло(о[3])), сНачалаГода: вПроцент(вЧисло(о[4])) };
}

// ── История значений инфляции ───────────────────────────────────────────
// Копим разобранные выпуски в файле рядом с server.js: по дате публикации.
// Так значения переживают перезапуск, а страницы Росстата читаются один раз.
const ИПЦ_ФАЙЛ = path.join(__dirname, 'ipc.json');
let ИПЦ_ИСТОРИЯ = { нед: {}, мес: {}, разобрано: {} };
try { Object.assign(ИПЦ_ИСТОРИЯ, JSON.parse(fs.readFileSync(ИПЦ_ФАЙЛ, 'utf8')) || {}); } catch (e) {}
function ипцСохранить() {
  try {
    fs.writeFileSync(ИПЦ_ФАЙЛ + '.tmp', JSON.stringify(ИПЦ_ИСТОРИЯ));
    fs.renameSync(ИПЦ_ФАЙЛ + '.tmp', ИПЦ_ФАЙЛ);
  } catch (e) {}
}
const знак = v => (v > 0 ? '+' : '') + v.toFixed(2).replace('.', ',') + '%';

// Обход раздела: берём ссылки на выпуски и разбираем те, что ещё не читали.
// Одну и ту же страницу второй раз не трогаем — помним её в «разобрано».
async function ипцОбойти(предел = 12) {
  const раздел = await fetchUrl(РОССТАТ_РАЗДЕЛ);
  const ссылки = ссылкиРосстата(раздел);
  let новых = 0, ошибок = 0;
  for (const с of ссылки) {
    if (ИПЦ_ИСТОРИЯ.разобрано[с.url]) continue;
    if (новых + ошибок >= предел) break;
    let html;
    try { html = await fetchUrl(с.url); } catch (e) { ошибок++; continue; }
    const н = разборИПЦ(html);
    const м = н ? null : разборИПЦМесяц(html);
    if (н) ИПЦ_ИСТОРИЯ.нед[с.дата] = знак(н.неделя);
    else if (м) ИПЦ_ИСТОРИЯ.мес[с.дата] = знак(м.кМесяцу);
    ИПЦ_ИСТОРИЯ.разобрано[с.url] = с.дата;
    новых++;
  }
  if (новых) ипцСохранить();
  return { ссылок: ссылки.length, прочитано: новых, ошибок,
           недель: Object.keys(ИПЦ_ИСТОРИЯ.нед).length, месяцев: Object.keys(ИПЦ_ИСТОРИЯ.мес).length };
}

// Расписание. Росстат выкладывает недельный выпуск по средам в 19:00, месячный
// — в первой половине месяца. Раз в час проверяем, не появилось ли новое: обход
// читает только незнакомые ссылки, поэтому лишней нагрузки нет.
setTimeout(() => { ипцОбойти(6).catch(() => {}); }, 40 * 1000);
setInterval(() => { ипцОбойти(6).catch(() => {}); }, 60 * 60 * 1000);

let ИПЦ = { данные: null, время: 0, адрес: '' };

async function обновитьИПЦ() {
  const раздел = await fetchUrl(РОССТАТ_РАЗДЕЛ);
  const ссылки = ссылкиРосстата(раздел);
  if (!ссылки.length) throw new Error('на странице раздела нет ссылок на выпуски');
  // Заголовок может обмануть, поэтому пробуем несколько верхних ссылок подряд
  // и берём первую, в которой действительно нашлась строка с оценкой.
  const промахи = [];
  for (const с of ссылки.slice(0, 6)) {
    let html;
    try { html = await fetchUrl(с.url); } catch (e) { промахи.push(с.url + ': ' + e.message); continue; }
    const д = разборИПЦ(html);
    if (д) { ИПЦ = { данные: д, время: Date.now(), адрес: с.url }; return ИПЦ; }
    промахи.push(с.url + ': нет строки с оценкой');
  }
  throw new Error('не нашёл выпуск с оценкой ИПЦ. Пробовал: ' + промахи.join(' | '));
}
...
const CAL_CBR = [
  ...
  ['2026-09-11', 'Решение по ключевой ставке', '',                          '', '14,00%', ''],
...
const CAL_CBR_ПОДБОРКА = 'https://rutube.ru/plst/106048/';
const CAL_CBR_ВИДЕО = {
  '2026-02-13': '7360abd607eea7fcb78b6f79f86b11d5',
...
// Объявления Минфина о завтрашних аукционах: дата вторника → какие выпуски.
// Заполняется сборщиком с сайта Минфина; руками сюда ничего не вписываем.
const OFZ_ОБЪЯВЛЕНИЯ = {
};

const CAL_OFZ_FACT = {
  // Сколько привлёк Минфин по номиналу. Спрос не показываем — в панели
  // важно само привлечение, чтобы «пред.» и «факт» были одной величиной.
  '2026-09-02': { fact: '1 000' },  // млрд руб. — единица показывается у названия
};

// ── Публикации Росстата ─────────────────────────────────────────────────
// Даты взяты из графика срочных информаций на rosstat.gov.ru/announcements.
// Считать их по дням недели нельзя: график сдвигается на праздниках, поэтому
// список ведём явно. Новый график выкладывают в конце июня и конце декабря.
// Время публикации в графике не указано; срочные информации выходят вечером.
const РОССТАТ_ВИДЫ = {
  ipcw: { name: 'Инфляция за неделю', hot: false, q: 'инфляц недел|недельн инфляц|росстат инфляц' },
  ipcm: { name: 'Инфляция за месяц', hot: true, q: 'инфляц месяц|ипц|индекс потребительск цен' },
  fuel: { name: 'Цены на нефтепродукты', hot: false, q: 'цен нефтепродукт|цен бензин дизель' },
  ppi: { name: 'Цены производителей', hot: false, q: 'цен производител|ппи' },
  ind: { name: 'Промышленное производство', hot: false, q: 'промышленн производств|промпроизводств' },
  gdp: { name: 'ВВП', hot: true, q: 'ввп росси|валов внутренн продукт' },
  gdpp: { name: 'ВВП, предварительная оценка', hot: true, q: 'ввп росси|предварительн оценк ввп' },
  gdpu: { name: 'Использование ВВП', hot: false, q: 'использован ввп' },
  benz: { name: 'Цены на бензин', hot: false, q: 'цен бензин|бензин подорожа' },
  prof: { name: 'Прибыль организаций', hot: false, q: 'прибыл организац|финансов результат' },
  zpl: { name: 'Долги по зарплате', hot: false, q: 'задолженност заработн плат|долг зарплат' },
  biz: { name: 'Деловая активность', hot: false, q: 'делов активност' },
  pexp: { name: 'Потребительские ожидания', hot: false, q: 'потребительск ожидан' },
  pov: { name: 'Границы бедности', hot: false, q: 'границ бедност|бедност росси' },
};
// Опубликованные значения инфляции — по дате выпуска. Нужны, чтобы у события
// стояло «пред.»: читателю важно сравнить с прошлой неделей или прошлым
// месяцем. Пополняется по мере выхода данных.
const CAL_ИПЦ_НЕД = {
  '2026-08-12': '-0,06%',   // 4–10 августа
  '2026-08-19': '-0,02%',   // 11–17 августа
  '2026-08-26': '+0,01%',   // 18–24 августа
  '2026-09-02': '-0,01%',   // 25–31 августа
};
const CAL_ИПЦ_МЕС = {
  '2026-02-13': '1,62%',    // январь
  '2026-03-13': '0,73%',    // февраль
  '2026-04-10': '0,60%',    // март
  '2026-05-15': '0,14%',    // апрель
  '2026-06-10': '0,17%',    // май
  '2026-07-10': '0,87%',    // июнь
  '2026-08-12': '0,54%',    // июль
};
// Значение ровно предыдущего выпуска. Берём именно соседний по графику, а не
// последний известный: иначе у всех будущих недель стояло бы одно и то же
// число, будто данные уже есть.
// Значения из истории главнее вписанных руками: они прочитаны с сайта.
function ипцТаблица(вид) {
  const база = вид === 'ipcw' ? CAL_ИПЦ_НЕД : CAL_ИПЦ_МЕС;
  const свежее = вид === 'ipcw' ? ИПЦ_ИСТОРИЯ.нед : ИПЦ_ИСТОРИЯ.мес;
  return Object.assign({}, база, свежее || {});
}

function calПрошлоеЗначение(табл, вид, дата) {
  const даты = new Set(Object.keys(табл));
  for (const [d, в] of CAL_ROSSTAT_ПРОШЛОЕ) if (в === вид) даты.add(d);
  for (const [d, в] of CAL_ROSSTAT) if (в === вид) даты.add(d);
  const все = [...даты].sort();
  const i = все.indexOf(дата);
  const пред = i > 0 ? все[i - 1] : '';
  return пред ? (табл[пред] || '') : '';
}

// Прошедшие публикации 2026 года — из графиков на I и II полугодие. Нужны,
// чтобы при прокрутке назад календарь не обрывался на текущей неделе.
const CAL_ROSSTAT_ПРОШЛОЕ = [
  ['2026-01-14','ipcw','1–12 января'],
  ['2026-01-14','fuel','1–12 января'],
  ['2026-01-16','ipcm','декабрь'],
  ['2026-01-21','ipcw','13–19 января'],
  ['2026-01-21','fuel','13–19 января'],
  ['2026-01-28','ppi','декабрь'],
  ['2026-01-28','ipcw','20–26 января'],
  ['2026-01-28','fuel','20–26 января'],
  ['2026-02-04','zpl','на конец декабря'],
  ['2026-02-04','benz','декабрь'],
  ['2026-02-04','ipcw','27 января – 2 февраля'],
  ['2026-02-04','fuel','27 января – 2 февраля'],
  ['2026-02-06','ind','2025 год'],
  ['2026-02-06','prof','январь–ноябре'],
  ['2026-02-11','biz','январь'],
  ['2026-02-11','ipcw','3–9 февраля'],
  ['2026-02-11','fuel','3–9 февраля'],
  ['2026-02-13','ipcm','январь'],
  ['2026-02-18','ppi','январь'],
  ['2026-02-18','ipcw','10–16 февраля'],
  ['2026-02-18','fuel','10–16 февраля'],
  ['2026-02-25','zpl','на конец января'],
  ['2026-02-25','benz','январь'],
  ['2026-02-27','ind','январь'],
  ['2026-02-27','ipcw','17–24 февраля'],
  ['2026-02-27','fuel','17–24 февраля'],
  ['2026-03-04','biz','февраль'],
  ['2026-03-04','ipcw','25 февраля – 2 марта'],
  ['2026-03-04','fuel','25 февраля – 2 марта'],
  ['2026-03-11','prof','2025 год'],
  ['2026-03-13','pov','IV квартал'],
  ['2026-03-13','ipcm','февраль'],
  ['2026-03-13','ipcw','3–10 марта'],
  ['2026-03-13','fuel','3–10 марта'],
  ['2026-03-18','ppi','февраль'],
  ['2026-03-18','ipcw','11–16 марта'],
  ['2026-03-18','fuel','11–16 марта'],
  ['2026-03-25','pexp','I квартал'],
  ['2026-03-25','zpl','на конец февраля'],
  ['2026-03-25','ind','январь–феврале'],
  ['2026-03-25','benz','февраль'],
  ['2026-03-25','ipcw','17–23 марта'],
  ['2026-03-25','fuel','17–23 марта'],
  ['2026-04-01','prof','январь'],
  ['2026-04-01','biz','март'],
  ['2026-04-01','ipcw','24–30 марта'],
  ['2026-04-01','fuel','24–30 марта'],
  ['2026-04-08','ipcw','31 марта – 6 апреля'],
  ['2026-04-08','fuel','31 марта – 6 апреля'],
  ['2026-04-10','ipcm','март'],
  ['2026-04-15','ipcw','7–13 апреля'],
  ['2026-04-15','fuel','7–13 апреля'],
  ['2026-04-22','ppi','март'],
  ['2026-04-22','zpl','на конец марта'],
  ['2026-04-22','ind','I квартал'],
  ['2026-04-22','ipcw','14–20 апреля'],
  ['2026-04-22','fuel','14–20 апреля'],
  ['2026-04-29','benz','март'],
  ['2026-04-29','prof','январь–феврале'],
  ['2026-04-29','biz','апрель'],
  ['2026-04-29','ipcw','21–27 апреля'],
  ['2026-04-29','fuel','21–27 апреля'],
  ['2026-05-06','ipcw','28 апреля – 4 мая'],
  ['2026-05-06','fuel','28 апреля – 4 мая'],
  ['2026-05-15','ipcm','апрель'],
  ['2026-05-15','gdpp','I квартал'],
  ['2026-05-15','ipcw','5–12 мая'],
  ['2026-05-15','fuel','5–12 мая'],
  ['2026-05-20','ppi','апрель'],
  ['2026-05-20','ipcw','13–18 мая'],
  ['2026-05-20','fuel','13–18 мая'],
  ['2026-05-27','zpl','на конец апреля'],
  ['2026-05-27','ind','январь–апреле'],
  ['2026-05-27','benz','апрель'],
  ['2026-05-27','ipcw','19–25 мая'],
  ['2026-05-27','fuel','19–25 мая'],
  ['2026-06-03','prof','I квартал'],
  ['2026-06-03','biz','май'],
  ['2026-06-03','ipcw','26 мая – 1 июня'],
  ['2026-06-03','fuel','26 мая – 1 июня'],
  ['2026-06-10','ipcm','май'],
  ['2026-06-10','pov','I квартал'],
  ['2026-06-10','ipcw',''],
  ['2026-06-10','fuel',''],
  ['2026-06-17','ppi','май'],
  ['2026-06-17','gdp','I квартал'],
  ['2026-06-17','ipcw','9–15 июня'],
  ['2026-06-17','fuel','9–15 июня'],
  ['2026-06-24','pexp','II квартал'],
  ['2026-06-24','zpl','на конец мая'],
  ['2026-06-24','ind','январь–мае'],
  ['2026-06-24','benz','май'],
  ['2026-06-24','ipcw','16–22 июня'],
  ['2026-06-24','fuel','16–22 июня'],
  ['2026-07-01','prof','январь–апреле'],
  ['2026-07-01','biz','июнь'],
  ['2026-07-01','gdpu','I квартал'],
  ['2026-07-01','ipcw','23–29 июня'],
  ['2026-07-01','fuel','23–29 июня'],
  ['2026-07-08','ipcw','30 июня – 6 июля'],
  ['2026-07-08','fuel','30 июня – 6 июля'],
  ['2026-07-10','ipcm','июнь'],
  ['2026-07-15','ipcw','7–13 июля'],
  ['2026-07-15','fuel','7–13 июля'],
  ['2026-07-22','ppi','июнь'],
  ['2026-07-22','zpl','на конец июня'],
  ['2026-07-22','ind',''],
  ['2026-07-22','ipcw','14–20 июля'],
  ['2026-07-22','fuel','14–20 июля'],
  ['2026-07-29','benz','июнь'],
  ['2026-07-29','prof','январь–мае'],
  ['2026-07-29','biz','июль'],
  ['2026-07-29','ipcw','21–27 июля'],
  ['2026-07-29','fuel','21–27 июля'],
  ['2026-08-05','ipcw','28 июля – 3 августа'],
  ['2026-08-05','fuel','28 июля – 3 августа'],
  ['2026-08-12','ipcm','июль'],
  ['2026-08-12','gdpp','II квартал'],
  ['2026-08-12','ipcw','4–10 августа'],
  ['2026-08-12','fuel','4–10 августа'],
  ['2026-08-19','ppi','июль'],
  ['2026-08-19','ipcw','11–17 августа'],
  ['2026-08-19','fuel','11–17 августа'],
  ['2026-08-26','zpl','на конец июля'],
  ['2026-08-26','ind','январь–июле'],
  ['2026-08-26','benz','июль'],
  ['2026-08-26','ipcw','18–24 августа'],
  ['2026-08-26','fuel','18–24 августа'],
  ['2026-09-02','biz','август'],
  ['2026-09-02','prof',''],
  ['2026-09-02','ipcw','25–31 августа'],
  ['2026-09-02','fuel','25–31 августа'],
];

const CAL_ROSSTAT = [
  ['2026-09-09','pov','II квартал'],
  ['2026-09-09','fuel','1–7 сентября'],
  ['2026-09-09','ipcw','1–7 сентября'],
  ['2026-09-11','gdp','II квартал'],
  ['2026-09-11','ipcm','август'],
  ['2026-09-16','fuel','8–14 сентября'],
  ['2026-09-16','ppi','август'],
  ['2026-09-16','ipcw','8–14 сентября'],
  ['2026-09-23','benz','август'],
  ['2026-09-23','fuel','15–21 сентября'],
  ['2026-09-23','ind','январь–августе'],
  ['2026-09-23','zpl','на конец августа'],
  ['2026-09-23','ipcw','15–21 сентября'],
  ['2026-09-23','pexp','III квартал'],
  ['2026-09-30','biz','сентябрь'],
  ['2026-09-30','fuel','22–28 сентября'],
  ['2026-09-30','prof','январь–июле'],
  ['2026-09-30','ipcw','22–28 сентября'],
  ['2026-10-02','gdpu','II квартал'],
  ['2026-10-07','fuel','29 сентября – 5 октября'],
  ['2026-10-07','ipcw','29 сентября – 5 октября'],
  ['2026-10-09','ipcm','сентябрь'],
  ['2026-10-14','fuel','6–12 октября'],
  ['2026-10-14','ipcw','6–12 октября'],
  ['2026-10-21','fuel','13–19 октября'],
  ['2026-10-21','zpl','на конец сентября'],
  ['2026-10-21','ppi','сентябрь'],
  ['2026-10-21','ipcw','13–19 октября'],
  ['2026-10-28','biz','октябрь'],
  ['2026-10-28','benz','сентябрь'],
  ['2026-10-28','fuel','20–26 октября'],
  ['2026-10-28','ind','январь–сентябре'],
  ['2026-10-28','prof','январь–августе'],
  ['2026-10-28','ipcw','20–26 октября'],
  ['2026-11-06','fuel','27 октября – 2 ноября'],
  ['2026-11-06','ipcw','27 октября – 2 ноября'],
  ['2026-11-11','fuel','3–9 ноября'],
  ['2026-11-11','ipcw','3–9 ноября'],
  ['2026-11-13','ipcm','октябрь'],
  ['2026-11-13','gdpp','III квартал'],
  ['2026-11-18','fuel','10–16 ноября'],
  ['2026-11-18','ppi','октябрь'],
  ['2026-11-18','ipcw','10–16 ноября'],
  ['2026-11-25','benz','октябрь'],
  ['2026-11-25','fuel','17–23 ноября'],
  ['2026-11-25','ind','январь–октябре'],
  ['2026-11-25','zpl','на конец октября'],
  ['2026-11-25','ipcw','17–23 ноября'],
  ['2026-12-02','biz','ноябрь'],
  ['2026-12-02','fuel','24–30 ноября'],
  ['2026-12-02','prof','январь–сентябре'],
  ['2026-12-02','ipcw','24–30 ноября'],
  ['2026-12-09','pov','III квартал'],
  ['2026-12-09','fuel','1–7 декабря'],
  ['2026-12-09','ipcm','ноябрь'],
  ['2026-12-09','ipcw','1–7 декабря'],
  ['2026-12-11','gdp','III квартал'],
  ['2026-12-16','fuel','8–14 декабря'],
  ['2026-12-16','ppi','ноябрь'],
  ['2026-12-16','ipcw','8–14 декабря'],
  ['2026-12-23','benz','ноябрь'],
  ['2026-12-23','fuel','15–21 декабря'],
  ['2026-12-23','ind','январь–ноябре'],
  ['2026-12-23','zpl','на конец ноября'],
  ['2026-12-23','ipcw','15–21 декабря'],
  ['2026-12-23','pexp','IV квартал'],
  ['2026-12-25','prof','январь–октябре'],
  ['2026-12-30','biz','декабрь'],
  ['2026-12-30','gdpu','III квартал'],
];
...
function calRules(начало, конец) {
  ...
  for (let d = new Date(d0); d <= d1; d.setDate(d.getDate() + 1)) {
    ...
    // Вторник — Минфин объявляет, какие выпуски пойдут на завтрашний аукцион.
    // Сообщение «О проведении аукционов» появляется во второй половине дня,
    // обычно около 15:45, но может задержаться до начала пятого.
    if (дн === 2) {
      const о = OFZ_ОБЪЯВЛЕНИЯ[ymd] || {};
      const прошлыйВторник = new Date(d.getTime() - 7 * 864e5)
        .toLocaleDateString('sv-SE', { timeZone: 'Europe/Moscow' });
      out.push({ id: 'ofzann-' + ymd, date: ymd, time: '15:45', kind: 'fin',
                 who: 'Минфин России', cc: 'ru', name: 'Проведение аукционов ОФЗ',
                 note: 'какие выпуски разместят в среду',
                 prev: (OFZ_ОБЪЯВЛЕНИЯ[прошлыйВторник] || {}).bonds || '',
                 fact: о.bonds || '', q: 'офз аукцион|минфин офз|проведен аукцион',
                 hot: false, noF: true });
    }
    // Среда — сами аукционы. Их два: первый подводят к двум часам, второй к
    // четырём. Раздельно не показываем: читателю важна итоговая сумма дня,
    // поэтому одна строка на 16:00 с суммарным привлечением.
    if (дн === 3) {
      ...
      out.push({ id: 'ofz-' + ymd, date: ymd, time: '16:00', kind: 'fin', who: 'Минфин России', cc: 'ru',
                 name: 'Аукционы ОФЗ', note: 'привлечено за день, оба аукциона',
                 prev: пред, fact: ф.fact || '',
                 q: 'офз аукцион|офз размещ', hot: false, unit: 'млрд руб.', noF: true });
    }
    ...
    // На праздничных неделях выпуск сдвигается — это отмечено в пояснении.
    {
      ...
      if (дн === 2)
        out.push({ id: 'api-' + ymd, date: ymd, time: летом ? '23:30' : '00:30', kind: 'stat',
                   ...
                   note: 'предварительные данные', q: 'запас нефт api|api запас', hot: false, unit: 'млн барр.' });
      if (дн === 3)
        out.push({ id: 'eia-' + ymd, date: ymd, time: летом ? '17:30' : '18:30', kind: 'stat',
                   ...
                   q: 'запас нефт сша|eia запас|запас нефт', hot: true, unit: 'млн барр.' });
    }
...
function calEvents(начало, конец) {
  ...
  for (const [d, name, note, f, prev, fact] of CAL_CBR) {
    ...
    // Пресс-конференция председателя идёт в тот же день в 15:00 и часто двигает
    // рынок сильнее самого решения. Отдельной строкой: у неё своё время.
    // Запись лежит на канале Банка России в RUTUBE — он встраивается и открыт
    // без VPN. Ссылка по дате; если её нет, кнопка ведёт на подборку целиком.
  ...
  for (const [d, вид, период] of [...CAL_ROSSTAT_ПРОШЛОЕ, ...CAL_ROSSTAT]) {
    const в = РОССТАТ_ВИДЫ[вид];
    if (!в) continue;
    const табл = (вид === 'ipcw' || вид === 'ipcm') ? ипцТаблица(вид) : null;
    const пред = табл ? calПрошлоеЗначение(табл, вид, d) : '';
    const факт = табл ? (табл[d] || '') : '';
    out.push({ id: 'ros-' + вид + '-' + d, date: d, time: '19:00', kind: 'stat', who: 'Росстат', cc: 'ru',
               name: в.name, note: период, q: в.q, hot: в.hot, prev: пред, fact: факт });
  }
...
const srv = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (u.pathname === '/api/ipc') {
      try {
        if (!ИПЦ.данные || Date.now() - ИПЦ.время > 30 * 60e3 || u.searchParams.get('force')) await обновитьИПЦ();
        return sendJson(req, res, { ok: true, адрес: ИПЦ.адрес, ...ИПЦ.данные });
      } catch (e) {
        return sendJson(req, res, { ok: false, ошибка: e.message }, { 'Cache-Control': 'no-store' });
      }
    }
    if (u.pathname === '/api/ipc/history') {
      try {
