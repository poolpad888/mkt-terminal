...
function calEvents(начало, конец) {
  ...
  for (const [d, вид, период] of [...CAL_ROSSTAT_ПРОШЛОЕ, ...CAL_ROSSTAT]) {
    ...
    if (!в) continue;
    const пред = вид === 'ipcw' ? calПрошлоеЗначение(CAL_ИПЦ_НЕД, вид, d)
               : вид === 'ipcm' ? calПрошлоеЗначение(CAL_ИПЦ_МЕС, вид, d) : '';
    const факт = вид === 'ipcw' ? (CAL_ИПЦ_НЕД[d] || '')
               : вид === 'ipcm' ? (CAL_ИПЦ_МЕС[d] || '') : '';
