...
const HARD_RE = [
  ['Банк России / ставка', /(?<![а-яё])цб(?![а-яё])|центробанк|банк[а-яё]?\s+росси|ключев[а-яё]+\s+ставк|(?<![а-яё])ставк[аеиу](?![а-яё])|набиуллин|заботкин|(?<![а-яё])цб\s*рф/i],
  ...
  ['кризис / обвал', /ядерн|обвал|крах|кризис|мобилизац|прекращени\s+поставок|дефицит\s+топлив/i],
];
const HARD_SIGNS = ['⚠️','💥','🔥','🚨'];
const SOFT_RE = [
  ['инфляция', /инфляц|(?<![а-яё])ипц(?![а-яё])|потребительск[а-яё]+\s+цен/i],
  ['рекорд', /рекорд|максимум|минимум/i],
  ['Мосбиржа', /мосбирж|московск[а-яё]+\s+бирж/i],
  ['крупный банк', /goldman|jpmorgan|morgan\s+stanley/i],
  ['налоги', /(?<![а-яё])налог|(?<![а-яё])ндс(?![а-яё])|(?<![а-яё])ндфл(?![а-яё])|(?<![а-яё])ндпи(?![а-яё])/i],
  ['нефть', /нефт|brent|urals/i],
  ['динамика', /повысил|понизил|запустит/i],
];
...
function classify(text) {
  ...
  if (lvl < 2) {
    let soft = 0;
    for (const [name, rx] of SOFT_RE) if (rx.test(text)) { soft++; if (reasons.length < 2) reasons.push(name); }
