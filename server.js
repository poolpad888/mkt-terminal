...
async function офзТик() {
  const t = мскЧасти();
  const окно = t.дн === 2 && t.минут >= 15 * 60 + 40 && t.минут <= 18 * 60 + 30;
  const пауза = окно ? 15 * 1000 : 30 * 60 * 1000;
  ...
  const день = t.дн === 2 ? t.ymd : сдвигДня(t.ymd, -((t.дн + 5) % 7));
  try { await офзОбъявление(день); } catch (e) {}
}
