...
// Сторож включаем ДО подключения. Прежний заводился уже после входа,
// поэтому зависшее подключение он не ловил: 5 сентября процесс полсуток
// перебирал попытки, а перезапуска никто не делал.
const СТАРТ = Date.now();
let подключено = false;
setInterval(() => {
  if (!подключено && Date.now() - СТАРТ > 300000) {
    console.log('пять минут не удаётся подключиться — перезапускаюсь');
    process.exit(1);
  }
}, 30000).unref();
...
(async () => {
  ...
  подключено = true;
  lastEvent = Date.now();
  console.log('подключение к Телеграму установлено');
  ...
  // Сторож: опрос идёт каждые несколько секунд и сам обновляет отметку,
  // так что десять минут без единого успешного круга — это уже поломка.
  setInterval(() => {
    if (Date.now() - lastEvent > 600000) {
      console.log('десять минут тишины — перезапускаюсь');
      process.exit(1);
    }
  }, 60000);
